...
n_train <- 1000 # size train data
n_test <- 200 # size test data/counterfactual data
SEED <- 123
...
data_no_confounder_train <- data_no_confounder_train[,-1] # remove id column
...
# Test data/no intervention
I <- node("A", distr = "rbern", prob = 0.5)
D_no_confounder_set <- D_no_confounder_set + action("I", nodes = I)
# Intervention -> class1
I1 <- node("A", distr = "rbern", prob = 1)
D_no_confounder_set <- D_no_confounder_set + action("I1", nodes = I1)
# Intervention -> class0
I0 <- node("A", distr = "rbern", prob = 0)
D_no_confounder_set <- D_no_confounder_set + action("I0", nodes = I0)

# Simulation test/counterfactual data
...
# Extract counterfactual data class = 0
...
# Extract counterfactual data class = 1
...
data_confounder_s_train[, C := NULL] # remove confounder

# Prepare intervention

# Test data/no intervention
I <- node("A", distr = "rbern", prob = plogis(0.5 + 0.7*C))
D_confounder_s_set <- D_confounder_s_set + action("I", nodes = I)
# Intervention -> class1
I1 <- node("A", distr = "rbern", prob = 1)
...
data_confounder_s_test[, C := NULL] # remove confounder
...
data_confounder_s_counterfactual_class0[, C := NULL] # remove confounder
...
data_confounder_s_counterfactual_class1[, C := NULL] # remove confounder
